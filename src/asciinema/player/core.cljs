...
(defn start-event-loop!
  ...
  (let [channels (atom initial-channels)]
    (go-loop []
      (let [[action channel] (alts! (seq @channels))]
        (when (nil? action)
          (swap! channels disj channel))

        (when (satisfies? a/Update action)
          (swap! player-atom #(a/update-player action %)))

        (when (satisfies? a/ChannelSource action)
          (swap! channels set/union (a/get-channels action @player-atom))))
      (recur))))
